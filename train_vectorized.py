...
def main():
    ...
    env = Maze(maze_size, num_agents ,max_steps=500, density=density)
    env = Monitor(env)
    if policy == 'CnnPolicy':
    ...
    name = agent_class.__name__ + f"_policy {policy}"+ str(f"fd{features_dim}" if policy=="CnnPolicy" else "") + "_lr" + str(lr) + "_net" + str(net_arch) + f"_maze{maze_size}x{maze_size}_agents{num_agents}_density({density})"
    agent = agent_class(policy,env , verbose=0, tensorboard_log=f"./tensorboard_vec_{maze_size}/", learning_rate=lr, policy_kwargs=policy_kwargs,)
    # agent.learn(total_timesteps=num_steps, log_interval=100, tb_log_name=name )
    agent.learn(total_timesteps=num_steps, log_interval=100, tb_log_name=name)
    # agent.save(f"./models/{agent_class.__name__}")
